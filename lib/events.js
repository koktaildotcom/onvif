...
module.exports = function(Cam) {
	...
	// A first attempt after a second keeps a device that fails every attempt well under the number of
	// authentications that makes some of them lock the account.
	const minEventReconnectMs = 1000;
				...
				if (!error) {
					...
					if (typeof error === 'object' && retryErrorCodes.includes(error.code)) {
						...
						this.unsubscribe(function(_err, _data, _xml) {
							// once the unsubscribe has completed (even if it failed), go around the loop again on
							// the increasing interval. A device that answers every pull with a fault would
							// otherwise be asked for a fresh subscription as fast as the network allows, and
							// since every attempt authenticates, some devices lock the account for too many
							// attempts. A pull point that merely expired is rebuilt after one interval, which the
							// next successful pull resets.
							this._restartEventRequest();
						}, true);
					}
				}
	...
	Cam.prototype._restartEventRequest = function() {
		...
		if (!this._eventReconnectms) {
			this._eventReconnectms = minEventReconnectMs;
		}
